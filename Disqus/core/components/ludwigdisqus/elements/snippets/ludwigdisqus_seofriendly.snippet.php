...
$activated = array(
	'plugin' => $modx->getOption( $pak . '.activated' ), 
...
} elseif ( $activated['seofriendly'] )
{
	...
	$res = $modx->query( "SELECT t.modx_id, c.author_name, c.comment, c.created, c.like, c.dislike, c.thread_id, c.id as comment_id, c.parent_id, c.isDeleted " . "FROM modx.disqus_comment c, modx.disqus_thread t " . "WHERE t.id=c.thread_id AND t.modx_id=" . intval( $modx_id ) );
	if ( is_object( $res ) )
	{
		...
		foreach( $data as $d )
		{
			if ( !$d['isDeleted'] )
			{
				$output .= $modx->getChunk( $properties['chunk'], array(
					...
					'comment' => nl2br( htmlspecialchars( $d['comment'], ENT_QUOTES ) ), 
