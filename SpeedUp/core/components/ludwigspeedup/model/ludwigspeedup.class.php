...
{
	private $modx;
	public $compress_stats= array('js' => NULL, 'css' => NULL, 'html' => NULL);
	...
	public function get_contenType_id( $type= array('text/javascript' => 0, 'text/css' => 0) )
	{
		...
		$query = $this->modx->newQuery('modContentType', array('mime_type:IN' => array_keys($type)));
		$query->sortby('id');
	...
	private function generate_url( $type= 'css', $query= '' )
	{
		...
		if ($this->seofriendly)
		{
			...
			$url= $url .'.php?'. http_build_query( array($type => $query), '', '&amp;');
		}
	...
	public function add_extra_assets()
	{
		...
		// Search for assets
		$new_scripts= array('text/javascript' => array(), 'text/css' => array() );
		$mime_types= array('link' => 'text/css', 'script' => 'text/javascript' );
		...
		if ( $this->modx->resource->setTVValue( $this->PKG_NAME_LOWER, json_encode($new_scripts) ) ) 
		{
			...
			// Generate URL
			$url_ary= array('link' => '/assets/components/ludwigspeedup/services.php?css='. $this->modx->resource->get('id') .'&amp;extra=1',
		...
		switch ( $mime_type )
		{
			...
			case 'text/javascript':
				$this->content= str_replace(	'</body>',
								'<script type="text/javascript" src="'. $url .'"></script></body>',
								$this->content );
				break;
			...
			case 'text/css':
				...
				if ($extra)
				{
					// CSS - Before BODY end
					$this->content= str_replace(	'</body>',
									'<link property="stylesheet" rel="stylesheet" href="'. $url .'" type="text/css"/></body>',
									$this->content );
	...
	public function get_service_content( $in_ary= array(), $mime_type= '' )
	{
		...
		// Get content
		$c= $this->modx->newQuery('modResource', array('id:IN' => $in_ary, 'contentType' => $mime_type ) );
	...
	public function is_cached( $url, $cache_it=false )
	{
		...
		$this->cache_options= array(	"opt" => array( xPDO::OPT_CACHE_KEY => $this->PKG_NAME, xPDO::OPT_CACHE_EXPIRES => $this->cache_expires ),
