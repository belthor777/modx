...
if ( $modx->getOption( $PKG_NAME_LOWER . '.activated', null, false, true ) )
{
	...
	$cache = array(
		"opt" => array(
			xPDO::OPT_CACHE_KEY => $PKG_NAME, 
			xPDO::OPT_CACHE_EXPIRES => $cache_expires
		), 
		"name" => $PKG_NAME_LOWER . '_' . md5( $url )
	);
			...
			if ( count( $counts ) > 0 )
			{
				...
				if ( $alias_url['host'] === $httpHost )
				{
					...
					$resource = $modx->getObject( 'modResource', array(
						'alias' => pathinfo( $alias_url['path'], PATHINFO_FILENAME )
					) );
					if ( !$resource->setTVValue( $PKG_NAME, $output ) )
					{
						$modx->log( xPDO::LOG_LEVEL_ERROR, 'There was a problem with TV ' . $PKG_NAME . '!' );
					}
				}
