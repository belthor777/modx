...
class LudwigMarkdown
{
	...
	// End Markdown: </pre>
	public function get_markdown( $output )
	{
		return( preg_replace(	"|<pre id=\"markdown\">(.*)</pre>|Uis",
	...
	public function generate_pandoc( $output= '' )
	{
		...
		preg_match_all( "|<pre id=\"markdown\">(.*)</pre>|Uis", $this->get_markdown( $output ), $matches );
		foreach( $matches[0] as $key => $val )
		{
			$text = $pandoc->runWith($matches[1][$key], $options);
			$output = str_replace($val,$text,$output);
		}
		return( $output );
	}
	...
	public function generate_phpmarkdown( $output= '' )
	{
		...
		foreach( $matches[0] as $key => $val )
		{
			$text = $md->defaultTransform($matches[1][$key]);
			$output = str_replace($val,$text,$output);
		}
	...
	public function generate_geshi( $output )
	{
		...
		foreach ( $search as &$item )
		{
			...
			{
				$output= preg_replace_callback( $item, 'geshiloader', $output );
			}
		}
