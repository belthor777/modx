...
class QRsvgOutput extends QRarea
{
	...
	public function getRawSvg()
	{
		...
		foreach( $this->paths as $path )
		{
			switch( $path[0] )
			{
				...
				case QR_AREA_TRACKER:
					...
					{
						$lib['tracker'] = '<symbol id="tracker"><path d="m 0 7 0 7 7 0 0 -7 -7 0 z m 1 1 5 0 0 5 -5 0 0 -5 z m 1 1 0 3 3 0 0 -3 -3 0 z" style="fill:' . $this->position_color . ';background-color:' . $this->back_color . ';stroke:none"></path></symbol>';
						if ( isset( $this->gcolor ) && ( $this->gcolor !== false ) )
						{
							$svg[] = $this->gcolor;
							$this->pstyle = $this->pstyle .'stroke:url(#gradient);';
						}					
					}
		...
		$svg[] = '<path d="' . join( ' ', $aggregate_paths ) . '" style="fill:' . $this->fore_color . ';background-color:' . $this->back_color . ';'. $this->pstyle .'" ></path>';
